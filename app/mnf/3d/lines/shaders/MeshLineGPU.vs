...
void main() {
	...
	vec4 prevPos = m * vec4( texture2D(positions,getPos(max(id-1.,0.))).rgb, 1.0 );
	vec4 nextPos = m * vec4( texture2D(positions,getPos(min(id+1.,offsetID+pointNumber-1.))).rgb, 1.0 );
